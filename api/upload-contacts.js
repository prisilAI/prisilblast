...
module.exports = async (req, res) => {
  ...
  const csvContent = req.body; // raw CSV string

  try {
    const response = await fetch(`${VPS_URL}/upload-contacts?sessionId=${sessionId}`, {
      method: 'POST',
      headers: { 'X-Secret': VPS_SECRET, 'Content-Type': 'text/plain' },
      body: csvContent
    });
